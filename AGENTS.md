...
Installs from the winget source in this fork request machine scope. A package can explicitly set "scope": "user" in config/applications.json when its WinGet manifest has no machine installer; Discord and Telegram Desktop are the approved exceptions. Keep Microsoft Store packages on their upstream path and review them separately for the personal selection. Do not silently retry a winget-source install in user scope when a package has no compatible machine installer.
